...
def plot_corr_yield_current_climate(df, var, climate_dict):
    """
    ...
    for n, item in enumerate(var):
        ...
        ax.set_title('Yield vs '+item+' : '+climate_dict[item], fontsize=50, fontweight='bold', y =  1.1)
    plt.tight_layout()
...
def panel_scatter(df, *args):
    """
    Panel of scatter plots of paired variables. 
    
    Parameters
    ----------
    df: dataframe
        dataframe containing the desired variables 
    
    args: lists of variables 
        each list contains one paired variables 
    ...
    nrows = len(args) // ncols + (len(args) % ncols > 0) # number of rows
    ...
    for n, pair in enumerate(args):
        ...
        ax.plot(var1, var2, 'o', markersize = 10, alpha = 0.7)
        ax.set_xlabel(pair[0], fontsize = 200)
        ax.set_ylabel(pair[1], fontsize = 200)
